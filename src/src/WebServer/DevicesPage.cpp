...
void handle_devices_TaskSettingsPage(taskIndex_t taskIndex, uint8_t page)
{
  ...
  else
  {
    ...
    #if FEATURE_PLUGIN_FILTER
    // TODO TD-er: Implement showing task filter settings
...
#if FEATURE_PLUGIN_FILTER
// TODO TD-er: Show task filter settings
...
void devicePage_show_task_values(taskIndex_t taskIndex, deviceIndex_t DeviceIndex)
{
  ...
  if (!Device[DeviceIndex].Custom && (valueCount > 0))
  {
...
#if FEATURE_PLUGIN_FILTER
    if (Device[DeviceIndex].PluginFilter)
    {
      html_table_header(F("Filter"), 30);
      ++colCount;
    }
#endif
#if FEATURE_PLUGIN_STATS
    ...
    for (uint8_t varNr = 0; varNr < valueCount; varNr++)
    {
      ...
      {
        html_TD();
        const String id = getPluginCustomArgName(F("TDFIL"), varNr); // ="taskdevicefilter"
        addCheckBox(id, ExtraTaskSettings.enabledPluginFilter(varNr));
      }
#endif
#if FEATURE_PLUGIN_STATS
      if (Device[DeviceIndex].PluginStats)
